...
class DownloadManager {
  ...
  Future<void> download(String url, String savePath, CancelToken cancelToken,
    ...
    _implementation.download(url, savePath, cancelToken,
  ...
  Future<DownloadTask?> addDownload(String url, String savedDir) =>
      _implementation.addDownload(url, savedDir);

  Future<void> pauseDownload(String url) => _implementation.pauseDownload(url);

  Future<void> cancelDownload(String url) =>
      _implementation.cancelDownload(url);

  Future<void> resumeDownload(String url) =>
      _implementation.resumeDownload(url);
  ...
  Future<void> pauseBatchDownloads(List<String> urls) async {
    await Future.wait(urls.map((element) => pauseDownload(element)));
  }

  Future<void> cancelBatchDownloads(List<String> urls) async {
    await Future.wait(urls.map((element) => cancelDownload(element)));
  }

  Future<void> resumeBatchDownloads(List<String> urls) async {
    await Future.wait(urls.map((element) => resumeDownload(element)));
  }
